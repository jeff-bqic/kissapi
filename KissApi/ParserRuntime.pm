...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::LineCounter;
...
;
    ...
    unless (exists $cache->{$from})
    {
        $parser->{lastlinenum} = $parser->{offsetlinenum}
          - KissApi::ParserRuntime::_linecount(substr($parser->{fulltext},$from))
          + 1;
...
sub resync   # ($linecounter)
{
    ...
    die "Tried to alter something other than a LineCounter\n"
        unless $self =~ /KissApi::ParserRuntime::LineCounter/;

    my $parser = $self->{parser};
    my $apparently = $parser->{offsetlinenum}
             - KissApi::ParserRuntime::_linecount(${$self->{text}})
             + 1;
...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::ColCounter;
...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::OffsetCounter;
...
sub ' . $namespace . '::' . $self->{"name"} .  '
{
    ...
    KissApi::ParserRuntime::_trace(q{Trying rule: [' . $self->{"name"} . ']},
    ...
    my $lastsep;
    my $current_match;
    my $expectation = new KissApi::ParserRuntime::Expectation(q{' . $self->expected() . '});
    $expectation->at($_[1]);
    '. ($parser->{_check}{thisoffset}?'
    my $thisoffset;
    tie $thisoffset, q{KissApi::ParserRuntime::OffsetCounter}, \$text, $thisparser;
    ':'') . ($parser->{_check}{prevoffset}?'
    my $prevoffset;
    tie $prevoffset, q{KissApi::ParserRuntime::OffsetCounter}, \$text, $thisparser, 1;
    ':'') . ($parser->{_check}{thiscolumn}?'
    my $thiscolumn;
    tie $thiscolumn, q{KissApi::ParserRuntime::ColCounter}, \$text, $thisparser;
    ':'') . ($parser->{_check}{prevcolumn}?'
    my $prevcolumn;
    tie $prevcolumn, q{KissApi::ParserRuntime::ColCounter}, \$text, $thisparser, 1;
    ':'') . ($parser->{_check}{prevline}?'
    my $prevline;
    tie $prevline, q{KissApi::ParserRuntime::LineCounter}, \$text, $thisparser, 1;
    ':'') . '
    my $thisline;
...
'
    unless ( $_matched || defined($score) )
    {
        ...
        KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' rule>>},
                     KissApi::ParserRuntime::_tracefirst($_[1]),
                     q{' . $self->{"name"} .'},
                     $tracelevel)
                    if defined $::RD_TRACE;
    ...
    if (defined $::RD_TRACE)
    {
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' rule<< (return value: [} .
                      $return . q{])}, "",
                      q{' . $self->{"name"} .'},
                      $tracelevel);
...
{
    my @items = @{$_[0]->{"items"}};
    if (@items==1 && ref($items[0]) =~ /\AKissApi::ParserRuntime::Error/
        && $items[0]->{commitonly} )
    {
        KissApi::ParserRuntime::_warn(2,"Lone <error?> in production treated
                        as <error?> <reject>");
        KissApi::ParserRuntime::_hint("A production consisting of a single
                      ...
                      '<error?> <reject>' KissApi::ParserRuntime
                      supplied it for you.");
        push @{$_[0]->{items}},
            KissApi::ParserRuntime::UncondReject->new(0,0,'<reject>');
    }
    elsif (@items==1 && ($items[0]->describe||"") =~ /<rulevar|<autoscore/)
    {
        ...
        ( ref($items[0]) =~ /\AKissApi::ParserRuntime::UncondReject/
        || ($items[0]->describe||"") =~ /<autoscore/
        ))
    {
        KissApi::ParserRuntime::_warn(1,"Optimizing away production: [". $_[0]->describe ."]");
        ...
        KissApi::ParserRuntime::_hint("The production starts with $what. That means that the
...
sub changesskip($)
{
    ...
    {
        if (ref($item) =~ /KissApi::ParserRuntime::(Action|Directive)/)
        {
...
sub addscore
{
    ...
    $self->additem(KissApi::ParserRuntime::Directive->new(
                  "local \$^W;
...
sub check_pending
{
    ...
    if ($self->{op})
    {
        ...
        {
        KissApi::ParserRuntime::_error("Incomplete <$next->{type}op:...>.", $line);
        KissApi::ParserRuntime::_hint(
...
sub enddirective
{
    ...
    unless ($self->{op})
    {
        KissApi::ParserRuntime::_error("Unmatched > found.", $line);
        KissApi::ParserRuntime::_hint(
    ...
    if ($op->{type} =~ /left|right/)
    {
        ...
        {
        push @{$self->{items}},
            KissApi::ParserRuntime::Operator->new(
...
sub prevwasreturn
{
    ...
    push @{$self->{items}},
        KissApi::ParserRuntime::Result->new();
}
...
'           if $parser->{_check}{itempos};
    ...
    if ($parser->{_AUTOACTION} && defined($item) && !$item->isa("KissApi::ParserRuntime::Action"))
    {
        $code .= $parser->{_AUTOACTION}->code($namespace,$rule);
        KissApi::ParserRuntime::_warn(1,"Autogenerating action in rule
                       \"$rule->{name}\":
                        $parser->{_AUTOACTION}{code}")
        and
        KissApi::ParserRuntime::_hint("The \$::RD_AUTOACTION was defined,
                      so any production not ending in an
                      explicit action has the specified
                      \"auto-action\" automatically
                      appended.");
    }
    elsif ($parser->{_AUTOTREE} && defined($item) && !$item->isa("KissApi::ParserRuntime::Action"))
    {
        ...
        KissApi::ParserRuntime::_warn(1,"Autogenerating tree-building action in rule
                       \"$rule->{name}\"")
        and
        KissApi::ParserRuntime::_hint("The directive <autotree> was specified,
    ...
    $code .=
'
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' production: ['
                      . $self->describe . ']<<},
                      KissApi::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
...
' . ( $parser->{_check}{itempos} ? '
        if ( defined($_itempos) )
        {
            KissApi::ParserRuntime::Production::_update_itempos($_itempos, $itempos[ 1], undef, [qw(from)]);
            KissApi::ParserRuntime::Production::_update_itempos($_itempos, $itempos[-1], undef, [qw(to)]);
        }
' : '' ) . '
...
'
        ...
        {
            KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' action>> (return value: [undef])})
                    if defined $::RD_TRACE;
            last;
        }
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' action<< (return value: [}
                      . $_tok . q{])},
                      KissApi::ParserRuntime::_tracefirst($text))
                        if defined $::RD_TRACE;
        push @item, $_tok;
...
'
        ...
        KissApi::ParserRuntime::_trace(q{Trying directive: ['
                    . $self->describe . ']},
                    KissApi::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
        ...
        if (defined($_tok))
        {
            KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' directive<< (return value: [}
                        . $_tok . q{])},
                        KissApi::ParserRuntime::_tracefirst($text))
                            if defined $::RD_TRACE;
        }
        ...
        {
            KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' directive>>},
                        KissApi::ParserRuntime::_tracefirst($text))
                            if defined $::RD_TRACE;
        }
...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::UncondReject;
...
'
        KissApi::ParserRuntime::_trace(q{>>Rejecting production<< (found '
                     . $self->describe . ')},
                     KissApi::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
...
sub code($$$$)
{
    ...
    {
        #WAS: $action .= "KissApi::ParserRuntime::_error(qq{$self->{msg}}" .  ',$thisline);';
    ...
    else      # GENERATE ERROR MESSAGE DURING PARSE
    {
        ...
        my $rule = $item[0];
           $rule =~ s/_/ /g;
        #WAS: KissApi::ParserRuntime::_error("Invalid $rule: " . $expectation->message() ,$thisline);
        push @{$thisparser->{errors}}, ["Invalid $rule: " . $expectation->message() ,$thisline];
        ';
    }

    my $dir =
          new KissApi::ParserRuntime::Directive('if (' .
...
sub new ($$$$$$)
{
    ...
    {
        KissApi::ParserRuntime::_warn(3, "Token pattern \"m$ldel$pattern$rdel$mod\"
...
my $code = '
        KissApi::ParserRuntime::_trace(q{Trying terminal: [' . $self->describe
                      . ']}, KissApi::ParserRuntime::_tracefirst($text),
                      q{' . $rule->{name} . '},
                      $tracelevel)
                        if defined $::RD_TRACE;
        ...
        . ($check->{itempos}? 'do {'.KissApi::ParserRuntime::Production::incitempos().' 1} and ' : '')
        . '  $text =~ m' . $ldel . '\A(?:' . $self->{"pattern"} . ')' . $rdel . $mod . ')
        {
            '.($self->{"lookahead"} ? '$text = $_savetext;' : '$text = $lastsep . $text if defined $lastsep;') .
            ($check->{itempos} ? KissApi::ParserRuntime::Production::unincitempos() : '') . '
            $expectation->failed();
...
'
        KissApi::ParserRuntime::_trace(q{Trying subrule: [' . $self->{"subrule"} . ']},
                  KissApi::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
        ...
        {
            '.($self->{"lookahead"} ? '$text = $_savetext;' : '').'
            KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' subrule: ['
            . $self->{subrule} . ']>>},
                          KissApi::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
                          $tracelevel)
        ...
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' subrule: ['
...
sub new ($$$$$$$$$$)
{
    ...
    if ($lookahead)
    {
        if ($min>0)
        {
           return new KissApi::ParserRuntime::Subrule($subrule,$lookahead,$line,$desc,$matchrule,$argcode);
        }
        else
        {
            KissApi::ParserRuntime::_error("Not symbol (\"!\") before
                        \"$subrule\" doesn't make
                        sense.",$line);
            KissApi::ParserRuntime::_hint("Lookahead for negated optional
                       repetitions (such as
...
'
        KissApi::ParserRuntime::_trace(q{Trying repeated subrule: [' . $self->describe . ']},
                  KissApi::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
        ...
        . ')))
        {
            KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' repeated subrule: ['
            . $self->describe . ']>>},
                          KissApi::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
                          $tracelevel)
        ...
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' repeated subrule: ['
                    . $self->{subrule} . ']<< (}
                    . @$_tok . q{ times)},
...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::Operator;
...
sub code($$$$)
{
    ...
    my $code = '
        KissApi::ParserRuntime::_trace(q{Trying operator: [' . $self->describe . ']},
                  KissApi::ParserRuntime::_tracefirst($text),
                  q{' . $rule->{"name"} . '},
                  $tracelevel)
                    if defined $::RD_TRACE;
...
';
        ...
        $code .= '
          ...
          {
              $itemposfirst = KissApi::ParserRuntime::Production::_duplicate_itempos($_itempos);
          }
' if $check->{itempos};
        ...
        $code .= '
          ...
          while ($repcount < ' . $self->{max} . ')
          {
            ...
            ' . (ref($op) eq 'KissApi::ParserRuntime::Token'
...
' if $check->{itempos};
        ...
        $code .= '
            ...
            ' . (ref($op) eq 'KissApi::ParserRuntime::Token' ? 'do { push @item, $item{'.($self->{name}||$self->{hashname}).'}=$1; } if defined $1;' : "" ) . '
          }
          $text = $savetext;
          pop @item if $backtrack;
...
';
    ...
    $code .= '
          ...
          {
              KissApi::ParserRuntime::Production::_update_itempos(
...
';
    ...
    $code .= '
        ...
        {
            KissApi::ParserRuntime::_trace(q{<<'.KissApi::ParserRuntime::_matchtracemessage($self,1).' operator: ['
                          . $self->describe
                          . ']>>},
                          KissApi::ParserRuntime::_tracefirst($text),
                          q{' . $rule->{"name"} .'},
        ...
        KissApi::ParserRuntime::_trace(q{>>'.KissApi::ParserRuntime::_matchtracemessage($self).' operator: ['
                      . $self->describe
                      . ']<< (return value: [}
                      . qq{@{$_tok||[]}} . q{]},
                      KissApi::ParserRuntime::_tracefirst($text),
...
package # this should not be indexed by CPAN
KissApi::ParserRuntime::Expectation;
...
sub _nextnamespace()
{
    return "KissApi::ParserRuntime::" . $nextnamespace++;
}
...
{
    my $class = ref($_[0]) || $_[0];
    local $KissApi::ParserRuntime::compiling = $_[2];
    my $name_space_name = defined $_[3]
        ? "KissApi::ParserRuntime::".$_[3]
        : _nextnamespace();
    my $self =
    {
        "rules"     => {},
        ...
        # wasn't present in some versions of KissApi::ParserRuntime used to
    ...
    if ($::RD_AUTOACTION) {
        ...
        $self->{_AUTOACTION}
            = new KissApi::ParserRuntime::Action($sourcecode,0,-1)
    }
...
sub Compile($$$$) {
    die "Compilation of KissApi::ParserRuntime grammars not yet implemented\n";
}

sub DESTROY {
    my ($self) = @_;
    my $namespace = $self->{namespace};
    $namespace =~ s/KissApi::ParserRuntime:://;
    if ($self->{_not_precompiled}) {
        ...
        # instead.
        {
            ...
            my $blank_sub = sub {};
            while (my ($name, $glob) = each %{"KissApi::ParserRuntime::$namespace\::"}) {
                *$glob = $blank_sub if defined &$glob;
            }
        }

        # Delete the namespace's stash
        delete $KissApi::ParserRuntime::{$namespace.'::'};
    }
}
...
sub _generate
{
    ...
    while (pos $grammar < length $grammar)
    {
            ...
            and do { ($code) = extract_codeblock($grammar); $code })
        {
            ...
            $item = new KissApi::ParserRuntime::Action($code,$lookahead,$line);
        ...
        elsif ($grammar =~ m/\G\s*<[^m]/gc)
        {
            ...
            elsif ($grammar =~ m/$UNCOMMITMK/gco)
            {
                ...
                $item = new KissApi::ParserRuntime::Directive('$commit=0;1',
            ...
            elsif ($grammar =~ m/$QUOTELIKEMK/gco)
            {
                ...
                $item = new KissApi::ParserRuntime::Directive(
                    'my ($match,@res);
            ...
            elsif ($grammar =~ m/$AUTOTREEMK/gco)
            {
                ...
                else
                {
                    ...
                    $self->{_AUTOTREE}{NODE}
                        = new KissApi::ParserRuntime::Action(q({bless \%item, ').$base.q('.$item[0]}),0,-1);
                    $self->{_AUTOTREE}{TERMINAL}
                        = new KissApi::ParserRuntime::Action(q({bless {__VALUE__=>$item[1]}, ').$base.q('.$item[0]}),0,-1);
                }
            }
            ...
            elsif ($grammar =~ m/$REJECTMK/gco)
            {
                ...
                $item = new KissApi::ParserRuntime::UncondReject($lookahead,$line,"<reject>");
                $prod and $prod->additem($item)
                       ...
                       } )
            {
                ...
                $item = new KissApi::ParserRuntime::UncondReject($lookahead,$line,$code);
                $prod and $prod->additem($item)
                      or  _no_rule($code,$line);
            }
        ...
        $self->{_AUTOACTION}
            = new KissApi::ParserRuntime::Action($code,0,-$line)
            }
            ...
            elsif ($grammar =~ m/$COMMITMK/gco)
            {
                ...
                $item = new KissApi::ParserRuntime::Directive('$commit = 1',
        ...
        $self->{__TRACE__} = $1 ? $2+0 : 1;
            ...
            elsif ($grammar =~ m/$AUTOERRORMK/gco)
            {
                ...
                $item = new KissApi::ParserRuntime::Error('',$lookahead,$1,$line);
                $prod and $prod->additem($item)
                    ...
                    $code })
            {
                ...
                $code =~ /\A\s*<error\??:(.*)>\Z/s;
                $item = new KissApi::ParserRuntime::Error($1,$lookahead,$commitonly,$line);
                $prod and $prod->additem($item)
                      or  _no_rule("$code",$line);
                $aftererror = !$commitonly;
            }
        ...
        elsif ($grammar =~ m/$RULE/gco)
        {
            ...
            if ($rulename =~ /Replace|Extend|Precompile|PrecompiledRuntime|Save/ )
            {
                _warn(2,"Rule \"$rulename\" hidden by method
                       KissApi::ParserRuntime::$rulename",$line)
                and
                _hint("The rule named \"$rulename\" cannot be directly
                       called through the KissApi::ParserRuntime object
                       for this grammar (although it may still
                       be used as a subrule of other rules).
                       It can't be directly called because
                       KissApi::ParserRuntime::$rulename is already defined (it
                       is the standard method of all
                       parsers).");
            }
            $rule = new KissApi::ParserRuntime::Rule($rulename,$self,$line,$replace);
            $prod->check_pending($line) if $prod;
            $prod = $rule->addprod( new KissApi::ParserRuntime::Production );
            $aftererror = 0;
        }
        elsif ($grammar =~ m/$UNCOMMITPROD/gco)
        {
            ...
            $prod->check_pending($line) if $prod;
            $prod = new KissApi::ParserRuntime::Production($line,1);
        ...
        elsif ($grammar =~ m/$ERRORPROD/gco)
        {
            ...
            _parseunneg("a new (error) production", $aftererror,
                    $lookahead,$line, substr($grammar, $-[0], $+[0] - $-[0]) ) or next;
            $prod->check_pending($line) if $prod;
            $prod = new KissApi::ParserRuntime::Production($line,0,1);
            $rule and $rule->addprod($prod)
                  or  _no_rule("<error>",$line);
            $aftererror = 0;
        }
        elsif ($grammar =~ m/$PROD/gco)
        {
            ...
            $rule
              ...
              and $prod = $rule->addprod(new KissApi::ParserRuntime::Production($line))
        ...
        elsif ($grammar =~ m/$LITERAL/gco)
        {
            ...
            $item = new KissApi::ParserRuntime::Literal($code,$lookahead,$line);
            $prod and $prod->additem($item)
        ...
        elsif ($grammar =~ m/$INTERPLIT/gco)
        {
            ...
            $item = new KissApi::ParserRuntime::InterpLit($1,$lookahead,$line);
            $prod and $prod->additem($item)
        ...
        elsif ($grammar =~ m/$TOKEN/gco)
        {
            ...
            $item = new KissApi::ParserRuntime::Token($1,'/',$3?$3:'',$lookahead,$line);
            $prod and $prod->additem($item)
...

        {
            ...
            $item = new KissApi::ParserRuntime::Token(@components[3,2,8],
                                 $lookahead,$line);
            $prod and $prod->additem($item)
        ...
        # EXTRACT TRAILING REPETITION SPECIFIER (IF ANY)
            ...
            if ($grammar =~ m/\G[(]/gc)
            {
                ...
                if ($grammar =~ m/$OPTIONAL/gco)
                {
                    ...
                    $item = new KissApi::ParserRuntime::Repetition($name,$1,0,1,
                                       $lookahead,$line,
                                       $self,
                                       $matchrule,
                                       $argcode);
                ...
                elsif ($grammar =~ m/$ANY/gco)
                {
                    ...
                    {
                        $item = new KissApi::ParserRuntime::Repetition($name,$1,0,$MAXREP,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$MANY/gco)
                {
                    ...
                    {
                        $item = new KissApi::ParserRuntime::Repetition($name,$1,1,$MAXREP,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$EXACTLY/gco)
                {
                    ...
                    {
                        $item = new KissApi::ParserRuntime::Repetition($name,$1,$1,$1,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
                ...
                elsif ($grammar =~ m/$BETWEEN/gco)
                {
                    ...
                    {
                        $item = new KissApi::ParserRuntime::Repetition($name,"$1..$2",$1,$2,
                                           $lookahead,$line,
                                           $self,
                                           $matchrule,
                                           $argcode);
            ...
            else
            {
                ...
                $item = new KissApi::ParserRuntime::Subrule($name,
                                       $lookahead,
                                       $line,
                                       $desc,
                                       $matchrule,
    ...
    unless ($ERRORS or $isimplicit or $KissApi::ParserRuntime::compiling)
    {
        my $code = $self->_code();
        if (defined $::RD_TRACE)
        {
    ...
    # CHECK FOR UNDEFINED RULES
        ...
        foreach $call ( @{$rule->{"calls"}} )
        {
            if (!defined ${$rules}{$call}
              &&!defined &{"KissApi::ParserRuntime::$call"})
            {
...
. qq!
    \${"AUTOLOAD"} =~ s/^$self->{namespace}/KissApi::ParserRuntime/;
    goto &{\${"AUTOLOAD"}};
}
}

!;
    $code .= "push \@$self->{namespace}\::ISA, 'KissApi::ParserRuntime';";
    $self->{"startcode"} = '';
...
# keep the output in a form suitable for KissApi::ParserRuntime.
#
...
sub _parserepeat($$$$$$$$$)    # RETURNS A REF TO AN ARRAY OF MATCHES
{
    ...
    if (defined $itemposfirst)
    {
        KissApi::ParserRuntime::Production::_update_itempos($_itempos, $itemposfirst, undef, [qw(from)]);
    }
...
sub _parse($$$$)
{
    ...
    my $errorprefix = "KissApi::ParserRuntime";
    $errortext =~ s/\s+/ /g;
    _write_ERROR($errorprefix, $errortext);
}
